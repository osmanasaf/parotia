...
class TVService:
    ...
    def get_user_tv_rating_for(self, user_id: int, tmdb_id: int) -> Optional[UserRatingResponse]:
        """Get user's rating for a specific TV show"""
        try:
            rating = self.rating_repo.get_user_rating(user_id, tmdb_id, "tv")
            return UserRatingResponse.from_orm(rating) if rating else None
        except Exception as e:
            logger.error(f"Error getting user tv rating for {tmdb_id}: {str(e)}")
            raise

    def get_user_tv_watchlist_with_ratings(self, user_id: int, status: Optional[str] = None) -> List[UserWatchlistWithRatingResponse]:
        """Get user's TV watchlist and enrich with user's rating if exists"""
        try:
            watchlist = self.watchlist_repo.get_user_watchlist(user_id, "tv", status)
            enriched: List[UserWatchlistWithRatingResponse] = []
            for item in watchlist:
                rating = self.rating_repo.get_user_rating(user_id, item.tmdb_id, "tv")
            ...
            logger.error(f"Error getting user tv watchlist with ratings: {str(e)}")
            raise
